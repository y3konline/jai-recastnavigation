...
__declspec(dllexport) void dtClosestPtPointTriangle(float* closest, const float* p,
...
__declspec(dllexport) bool dtClosestHeightPointTriangle(const float* p, const float* a, const float* b, const float* c, float& h);

__declspec(dllexport) bool dtIntersectSegmentPoly2D(const float* p0, const float* p1,
							  const float* verts, int nverts,
							  float& tmin, float& tmax,
							  int& segMin, int& segMax);

__declspec(dllexport) bool dtIntersectSegSeg2D(const float* ap, const float* aq,
...
__declspec(dllexport) bool dtPointInPolygon(const float* pt, const float* verts, const int nverts);

__declspec(dllexport) bool dtDistancePtPolyEdgesSqr(const float* pt, const float* verts, const int nverts,
							float* ed, float* et);

__declspec(dllexport) float dtDistancePtSegSqr2D(const float* pt, const float* p, const float* q, float& t);
...
__declspec(dllexport) void dtCalcPolyCenter(float* tc, const unsigned short* idx, int nidx, const float* verts);
...
__declspec(dllexport) bool dtOverlapPolyPoly2D(const float* polya, const int npolya,
...
__declspec(dllexport) void dtRandomPointInConvexPoly(const float* pts, const int npts, float* areas,
							   const float s, const float t, float* out);

template<typename TypeToRetrieveAs>
__declspec(dllexport) TypeToRetrieveAs* dtGetThenAdvanceBufferPointer(const unsigned char*& buffer, const size_t distanceToAdvance)
{
...
template<typename TypeToRetrieveAs>
__declspec(dllexport) TypeToRetrieveAs* dtGetThenAdvanceBufferPointer(unsigned char*& buffer, const size_t distanceToAdvance)
{
	TypeToRetrieveAs* returnPointer = reinterpret_cast<TypeToRetrieveAs*>(buffer);
	buffer += distanceToAdvance;
	return returnPointer;
